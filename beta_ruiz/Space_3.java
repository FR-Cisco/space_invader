...
public class Space_3 extends JPanel
{ 
   ...
   public void fillBottomEnemies()
   {
      for (int x = 0; x < easyArray.length; x++)
      {
         if(x < 11)
         {
            easyArray[x] = new EasyEnemy(BFrame, 80*x, 0, x, 3);
            mediumArray[x] = new MediumEnemy(BFrame, 80*x, 0, x, 3);
         }
         else
         {
            easyArray[x] = new EasyEnemy(BFrame, 80*(x-11), 45,x, 3);
            mediumArray[x] = new MediumEnemy(BFrame, 80*(x-11), -45, x, 3);
         }
      }
   }
   ...
   public void fillHardArray()
   {
      ...
      {
         hardArray[x] = new HardEnemy(BFrame, 80*x, -45, x, 3);
      }      
   }
   ...
   public void checkInvade()
   {
      for(int x = 0; x < easyArray.length; x++)
      {
         if( easyArray[x] != null && easyArray[x].get_Y() >= 480)
         {
            setPause(true);
            go = new GameOver("" + score, 10);
            gameDone = true;
         }
         if( mediumArray[x] != null && mediumArray[x].get_Y() >= 480)
         {
            setPause(true);
            go = new GameOver("" + score, 10);
            gameDone = true;
      ...
      for(int x = 0; x < hardArray.length; x++)
      {
         if(hardArray[x] != null && hardArray[x].get_Y() >= 457)
         {
            setPause(true);
            go = new GameOver("" + score, 10);
            gameDone = true;
